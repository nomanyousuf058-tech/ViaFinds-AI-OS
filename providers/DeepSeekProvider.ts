...
export class DeepSeekProvider extends BaseProvider {
  ...
  public async initialize(): Promise<void> {
    if (this.config.disabled) {
      logger.info(`DeepSeekProvider is Disabled`);
      return;
    }
    if (!this.config.apiKey) {
      logger.info(`DeepSeekProvider is Missing API Key`);
      return;
    }
    logger.info(`DeepSeekProvider initialized with model: ${this.config.defaultModel}`);
  }

  public async generateCompletion(payload: AIPromptPayload): Promise<AIProviderResponse> {
    if (this.config.disabled) {
      throw new Error('DeepSeekProvider is disabled');
    }
    if (!this.config.apiKey) {
      throw new Error('DeepSeekProvider API key is not configured');
    }
