...
export class OpenRouterProvider extends BaseProvider {
  ...
  public async initialize(): Promise<void> {
    if (this.config.disabled) {
      logger.info(`OpenRouterProvider is Disabled`);
      return;
    }
    if (!this.config.apiKey) {
      logger.info(`OpenRouterProvider is Missing API Key`);
      return;
    }
    logger.info(`OpenRouterProvider initialized with model: ${this.config.defaultModel}`);
  }

  public async generateCompletion(payload: AIPromptPayload): Promise<AIProviderResponse> {
    if (this.config.disabled) {
      throw new Error('OpenRouterProvider is disabled');
    }
    if (!this.config.apiKey) {
    ...
    const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      headers: {
        ...
        'HTTP-Referer': 'https://viafinds.com', // Optional but good practice for OpenRouter
        'X-Title': 'ViaFinds AI OS',
      },
      body: JSON.stringify({
    ...
    try {
      const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
        ...
        headers: {
          'Authorization': `Bearer ${this.config.apiKey}`,
          'Content-Type': 'application/json',
          'HTTP-Referer': 'https://viafinds.com',
          'X-Title': 'ViaFinds AI OS',
        },
        body: JSON.stringify({
          model: this.config.defaultModel,
